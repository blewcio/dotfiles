

# Install productivity helpers
cask "rectangle"      # Move windows around with keys
cask "alt-tab"        # Windows like tab
cask "alfred"         # Additional search and much more
cask "pock"           # Touchbar config
cask "cheatsheet"      # Show shotcuts on screen
cask  customssshortcut # Define own shortcuts in Apps


# Install basic tools
cask "iterm2" 
cask "macvim" 
cask "google-chrome" 
cask "amazon-photos"  
cask "messenger" 
cask "miro" 
cask "zoom" 
cask "gimp" 
 
# Install basic tools for improved terminal
brew "git"        
brew "wget"       
brew "bat"          # Improved cat
brew "exa"          # Improved ls
brew "tree"       
brew "tldr"         # Improved man
brew "jq"             # Commandline JSON processor
brew "htop"         # Improved top
brew "jdupes"      # Duplicate file finder
brew "fzf"           # Fuzzy finder
brew "fasd"         # Quick access to (recent) files and directories
brew "ripgrep"     # Improved grep
brew "tmux"        # Improvee screen
brew "wakeonlan"  
brew "nmap"        # Network scanner
brew "ranger"      # Vi like file browser
brew "mas"          #install from Appstore via Terrminal)
brew "speedtesst-cli"  Network speed test
brew "terminal-notifier"  # Notify via notification center
brew "ffmpeg"

# Install quick look plugins
cask "qlcolorcode" 
cask "qlstephen" 
cask "qlmarkdown" 
cask "qlvideo" 
cask "quicklook-json" 
cask "qlprettypatch" 
cask "quicklook-csv" 
cask "suspicious-package" 
cask "betterzip" 
cask "webpquicklook"

# Apps from Appstore
mas "HomeAtmo Lite", id: 1437630108  # HomeAtmo Lite
mas "QuickCal", id: 416581096   # QuickCal for widget in notification center
mas "Brrother P Touch", id: 1453365242  # Brothere P Touch
mas "We Love Lights", id: 1534707384  # Hue App or we love lights
mas "JustStream", id: 1474804779  # Juststream for file to Chromecast streaming

# Install additional tools
cask "appcleaner" 
cask "mysides"  # Simple CLI based sidebar configuration
cask "brave-browser"  
cask "caffeine"  # Dont let mac sleep
cask "android-file-transfer" 
cask "anydo" 

brew "gnumeric"
 
 
 
 
 
 
 
